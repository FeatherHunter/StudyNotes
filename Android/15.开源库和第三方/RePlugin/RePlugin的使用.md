...
8、无论是内置，还是外置插件，不是所有的APK都能作为 RePlugin 的插件并安装进来的。
...
11、如果插件需要下载，需要覆写`RePluginCallbacks.onPluginNotExistsForActivity`方法，并在此打开自定义的下载页面并控制其逻辑
...
31、出于性能考虑，内置插件无需做“签名校验”，仅“外置插件”会做.
> 一旦签名校验失败，则会在Logcat中提示`“verifySignature: invalid cert”`，且install方法返回null。

32、若在调用 install 方法前就已对APK做了校验，则可关闭，以避免重复校验。
...
34、目前有两种进程可以作为“插件管理进程”：
> 1. 常驻进程
> 1. 主进程

##### 常驻进程

35、默认以“常驻进程”作为“插件管理进程”
...
41、以“主进程”作为“插件管理进程”的优点
> 1. 无需额外启动任何进程.
> 1. 应用冷启动的时间会短一些，因为无需再拉起额外进程

42、以“主进程”作为“插件管理进程”的缺点
...
45、为了保证稳定性，会把经过验证的插件放到一个特殊的目录下，以防止“源文件”被删除后的一些问题。
...
47、外置插件：为了方便使用，插件会有一个JSON文件，用来记录所有已安装插件的信息。
> 位于“主程序路径/app_p_a/p.l”中。

48、内置插件：内置插件的JSON文件只存放于主程序“assets/plugins-builtin.json”文件下。每次会从那里获取信息。
