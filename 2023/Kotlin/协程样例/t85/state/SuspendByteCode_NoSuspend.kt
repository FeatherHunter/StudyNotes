...
// TODO >>>>>>>>>>>>>>>>>>>>>>>>  t85：协程背后状态机原理 ⑤ >>>>>>>>>>>>>>>>>>>>>>>>
fun showCoroutine2(continuation: Continuation<Any?>): Any? {

    class ShowContinuation2(continuation: Continuation<Any?>) : ContinuationImpl(continuation) {
    ...
    val continuationState: ShowContinuation2 = if (continuation is ShowContinuation2) {
        continuation
    } else {
        ShowContinuation2(continuation) // 如果是初次运行，会把continuation: Continuation<Any?>作为参数传递进去
    }
    ...
    while (loop) {
        ...
        when (continuationState.label) {
            0 -> {
                ...
                // 执行 requestLoadUser
                suspendReturn = requestLoadUser_NoSuspend(continuation) // withContent(IO)执行耗时任务
                ...
                if (suspendReturn == sFlag) {
                    ...
                    // 到这里为止，全部结束
                    // 等等耗时任务完成后，调用invokeSuspend 马上进入下一个专题
                }
            }
            ...
            1 -> {
                ...
                suspendReturn = requestLoadUserAssets_NoSuspend(continuation) // withContent(IO)执行耗时任务
            ...
            2 -> {
                ...
                suspendReturn = requestLoadUserAssetsDetails_NoSuspend(continuation) // withContent(IO)执行耗时任务
...
private fun requestLoadUser_NoSuspend(completion: Continuation<String>): Any? {
    // no implement
    // 调用 invokeSuspend
    try {
...
private fun requestLoadUserAssets_NoSuspend(completion: Continuation<String>): Any? {
    // no implement
    // 调用 invokeSuspend
    try {
...
private fun requestLoadUserAssetsDetails_NoSuspend(completion: Continuation<String>): Any? {
    // no implement
    // 调用 invokeSuspend
    try {
